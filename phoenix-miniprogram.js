/**
 * Phoenix Channels Miniprogram client
 *
 * ## Socket Connection
 *
...
let Serializer = {
  ...
  decode(rawPayload, callback) {
    let [joinRef, ref, topic, event, payload] = JSON.parse(rawPayload)
    const options = { join_ref: joinRef, ref, topic, event, payload }
...
export class Socket {
  ...
  endPointURL() {
    let uri = UrlHelper.appendParams(
      UrlHelper.appendParams(this.endPoint, this.params), { vsn: VSN })
    if (uri.charAt(0) !== '/') {
      return uri
    }
    return `wss://${uri}`
  ...
  connect() {
    return new Promise((resolve, reject) => {
      if (this.conn) { return }
      this.conn = new MpWebsocket(this.endPointURL(), resolve, reject)
  ...
  push(data) {
    let { topic, event, payload, ref, join_ref: joinRef } = data
    let callback = () => {
      this.encode(data, result => {
        this.conn.send(result)
      })
    }
    this.log('push', `${topic} ${event} (${joinRef}, ${ref})`, payload)
    if (this.isConnected()) {
      callback()
    } else {
  ...
  onConnMessage(rawMessage) {
    this.decode(rawMessage.data, msg => {
      let { topic, event, payload, ref, join_ref: joinRef } = msg
      if (ref && ref === this.pendingHeartbeatRef) { this.pendingHeartbeatRef = null }

      this.log('receive', `${payload.status || ''} ${topic} ${event} ${(ref && '(' + ref + ')') || ''}`, payload)
      this.channels.filter(channel => channel.isMember(topic, event, payload, joinRef))
...
class UrlHelper {
  static appendParams(url, params) {
    ...
    return `${url}${prefix}${UrlHelper.serialize(params)}`
  }
  ...
  static serialize(obj, parentKey) {
    ...
    for (var key in obj) {
      ...
      if (typeof paramVal === 'object') {
        queryStr.push(UrlHelper.serialize(paramVal, paramKey))
      } else {
...
export class MpWebsocket {
  constructor(endPoint, resolve, reject) {
    this.instance = wx.connectSocket({ url: endPoint })
